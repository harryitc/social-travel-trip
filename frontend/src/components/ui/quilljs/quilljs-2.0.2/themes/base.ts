...
import { resizeBase64Image } from '../extensions/resize_image';
...
class BaseTheme extends Theme {
  ...
  constructor(quill: Quill, options: ThemeOptions) {
    super(quill, options);
    const listener: EventListener = (mouseEvent: Event) => {
      ///Casting type
    ...
    toolbar: {
      handlers: {
        formula() {
          ...
          _this.quill.theme.tooltip.edit('formula');
        },
          ...
          if (fileInput == null) {
            ...
            fileInput.addEventListener('change', async () => {
              if (fileInput.files != null && fileInput.files[0] != null) {
                ...
                const resizedImageBase64 = await resizeBase64Image(file, file.type);
                let range = module.quill.getSelection(true);
                module.quill.updateContents(
                  new Delta()
                    .retain(range.index)
                    .delete(range.length)
                    .insert({ image: resizedImageBase64 }),
                  Emitter.sources.USER,
                );
                module.quill.setSelection(range.index + 1, Emitter.sources.SILENT);
                fileInput.value = '';
              }
            });
        ...
        //Show video input popup
        image: function () {
          const module = this as any;
          module.quill.theme['tooltip'].edit('image');
        },
        video() {
          const _this = this as any;
          _this.quill.theme.tooltip.edit('video');
        },
      },
    },
  },

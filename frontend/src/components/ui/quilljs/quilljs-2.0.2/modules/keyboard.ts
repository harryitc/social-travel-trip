...
import Delta, { AttributeMap, Op } from '../lib/quill-delta-5.1.0/Delta';
import { EmbedBlot, Scope, TextBlot } from '../lib/parchment-3.0.0/parchment';
import { Blot, BlockBlot } from '../lib/parchment-3.0.0/parchment';
...
class Keyboard extends Module<KeyboardOptions> {
  ...
  listen() {
    this.quill.root.addEventListener('keydown', (evt) => {
      ...
      const prefixText =
        leafStart instanceof TextBlot ? leafStart.value().slice(0, offsetStart) : '';
  ...
  /**
   * Fix focus not working properly
   * @param line
   */
  fixFocus(line: Blot, rangeIndex?: number) {
    if (line instanceof TableCell) {
      TableCell.fixFocus(line, () => {
        // @ts-expect-error Fix me later
        this.quill.setSelection(rangeIndex ?? this.quill.getSelection().index - 1);
        this.quill.focus();
      });
    }
  }
  ...
  handleDeleteRange(range: Range) {
    ...
    const [line] = this.quill.getLine(range.index);
    // @ts-expect-error Fix me later
    this.fixFocus(line, range.index);
  }
  ...
  bindings: {
    ...
    'table delete': {
      ...
      handler: function (range, context) {
        const module = this as Module;
        const [tableCell, offset] = module.quill.getLine(range.index);
        // @ts-expect-error Fix me later
        const table = tableCell.parent;
        if (table.children.length === 1) {
          module.quill.format('table', false, Quill.sources.USER);
          ///Stop default
          return false;
        }
        ///Default
    ...
    'table enter': {
      ...
      handler: function (range) {
        ...
        const table = tableCell.parent;
        let pos = 0;
        if (tableCell === table.children.head) {
          pos = -1;
        }
        if (tableCell === table.children.tail) {
          pos = 1;
        }
        if (pos === 0) {
          return;
        ...
        let delta = new Delta();
        // @ts-expect-error Fix me later
        const tableCellIndex = module.quill.getIndex(tableCell);
        let newPos = 0;
        ///Add new line at the end of the table
        if (pos === 1) {
          // @ts-expect-error Fix me later
          newPos = tableCellIndex + tableCell['cache'].length;
          delta = delta.retain(newPos).insert('\n');
          module.quill.updateContents(delta);
        }
        ///Add new line at the beginning of the table
        if (pos === -1) {
          newPos = tableCellIndex;
          delta = delta.retain(tableCellIndex - 1).insert('\n');
          module.quill.updateContents(delta);
        }
        module.quill.setSelection(newPos);
        module.quill.scrollSelectionIntoView();
      },
    },
  },
};
...
function makeEmbedArrowHandler(key: string, shiftKey: boolean | null): BindingObject {
...
function normalize(binding: Binding): BindingObject | null {
  ...
  return binding as BindingObject;
}

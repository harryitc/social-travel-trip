import { ScrollBlot } from '../lib/parchment-3.0.0/parchment';
import {
  Attributor,
  BlockBlot,
  ClassAttributor,
  ...
  Scope,
  StyleAttributor,
} from '../lib/parchment-3.0.0/parchment';
...
class Clipboard extends Module<ClipboardOptions> {
  ...
  convert({ html, text }: { html?: string; text?: string }, formats: Record<string, unknown> = {}) {
    ...
    this.matchers.forEach((pair) => {
      ...
      switch (selector) {
        ...
        case Node.ELEMENT_NODE:
          ...
          break;
        default:
          Array.from(container.querySelectorAll(selector as string)).forEach((element) => {
...
function applyFormat(delta: Delta, format: string, value: unknown, scroll: ScrollBlot): Delta {
...
function createMatchAlias(format: string) {
  return (element: Node, delta: Delta, scroll: ScrollBlot) => {
    const node = element as Element;
    return applyFormat(delta, format, true, scroll);
  };
}

function matchAttributor(htmlElement: Node, delta: Delta, scroll: ScrollBlot) {
  ///
  const node = htmlElement as HTMLElement;
...
function matchStyles(htmlElement: Node, delta: Delta, scroll: ScrollBlot) {
  const node = htmlElement as HTMLElement;
...
function matchTable(htmlTableRowElement: Node, delta: Delta, scroll: ScrollBlot) {
  const node = htmlTableRowElement as HTMLTableRowElement;
  const table =
...
function matchText(htmlElement: Node, delta: Delta, scroll: ScrollBlot) {
  ///Cast type
  const node = htmlElement as HTMLElement;
  // @ts-expect-error
  let text = node.data;
  ...
  if (!isPre(node)) {
    if (
      text.trim().length === 0 &&
      text.includes('\n') &&
      !isBetweenInlineElements(node as HTMLElement, scroll)
    ) {
      return delta;
    }

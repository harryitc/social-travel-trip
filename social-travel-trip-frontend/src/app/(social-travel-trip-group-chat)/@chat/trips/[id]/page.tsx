...
/**
 * Parallel Route: @chat/trips/[id]
 * Center column - Chat area for specific trip
 */
export default function ChatDetailPage() {
  ...
  useEventListeners({
    'group:selected': (data) => {
      console.log('💬 [ChatDetailPage] Group selected:', data.group.id, data.group.title);
      setSelectedGroup(data.group);
    },
    'group:updated': (data) => {
      // Update group info if it's the currently selected group
      if (selectedGroup && selectedGroup.id === data.group.id) {
        console.log('💬 [ChatDetailPage] Group updated:', data.group.title);
        setSelectedGroup(data.group);
      }
    },
  });

import { ContainerBlot, LeafBlot, Scope, ScrollBlot } from '../lib/parchment-3.0.0/parchment';
import { Blot, Parent, EmbedBlot, ParentBlot, Registry } from '../lib/parchment-3.0.0/parchment';
...
class Scroll extends ScrollBlot {
  ...
  insertBefore(blot: Blot, ref?: Blot | null) {
    if (blot.statics.scope === Scope.INLINE_BLOT) {
      const wrapper = this.scroll.create(this.statics.defaultChild.blotName) as Parent;
  ...
  insertContents(index: number, delta: Delta) {
    ...
    if (first) {
      ...
      const delta =
        first.type === 'block' ? first.delta : new Delta().insert({ [first.key]: first.value });
  ...
  lines(index = 0, length = Number.MAX_VALUE): (Block | BlockEmbed)[] {
    const getLines = (blot: ParentBlot, blotIndex: number, blotLength: number) => {
...
function insertInlineContents(parent: ParentBlot, index: number, inlineContents: Delta) {

import { LeafBlot, Scope } from '../lib/parchment-3.0.0/parchment';
...
class Selection {
  ...
  constructor(scroll: Scroll, emitter: Emitter) {
    ...
    this.emitter.on(Emitter.events.SCROLL_BEFORE_UPDATE, () => {
      ...
      this.emitter.once(Emitter.events.SCROLL_UPDATE, (source, mutations: MutationRecord[]) => {
        try {
  ...
  rangeToNative(range: Range): [Node | null, number, Node | null, number] {
    ...
    const getPosition = (index: number, inclusive: boolean): [Node | null, number] => {

...
import { Blot, ScrollBlot } from '../lib/parchment-3.0.0/parchment';
...
const copyIconSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" width="18px" height="18px"><path d="M0 0h24v24H0z" fill="none"/><path fill="white" d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>`;
...
class SyntaxCodeBlockContainer extends CodeBlockContainer {
  ...
  highlight(highlight: (text: string, language: string) => Delta, forced = false) {
          ...
          if (attributes) {
            Object.keys(attributes).forEach((format) => {
              if ([SyntaxCodeBlock.blotName, CodeToken.blotName].includes(format)) {
                //* @ts-expect-error
                this.formatAt(index, retain as number, format, attributes[format]);
              }
            });
          }
          //* @ts-expect-error
          return index + (retain as number);
        }, 0);
      }
  ...
  optimize(context: Record<string, any>) {
    ...
    if (this.parent != null && this.children.head != null && this.uiNode != null) {
      const language = SyntaxCodeBlock.formats(this.children.head.domNode);
      const select = this.uiNode.lastChild;
      // @ts-expect-error
...
class Syntax extends Module<SyntaxOptions> {
  ...
  constructor(quill: Quill, options: Partial<SyntaxOptions>) {
    ...
    if (this.options.hljs == null) {
      throw new Error(
        'Syntax module requires highlight.ts. Please include the library on the page before Quill.',
      );
    }
  ...
  initListener() {
    this.quill.on(Quill.events.SCROLL_BLOT_MOUNT, (blot: Blot) => {
      ...
      //* @ts-expect-error Fix me later
      CodeBlockHighlighter.languages.keys.forEach((key: string) => {
        const option = select.ownerDocument.createElement('option');
        option.textContent = CodeBlockHighlighter.languages.list[key].label;
      ...
      copyBtn.addEventListener('click', () => {
        const html = blot.getCode();
        navigator.clipboard.writeText(html);
      });

      ///Wrap
      const uiNode = document.createElement('div');
      uiNode.append(select);
      //Attach
      if (blot.uiNode == null) {
        blot.attachUI(uiNode);

---
to:  <%= angularOutputPath %>/<%= h.changeCase.kebabCase(moduleName)%>/components/create/create.component.ts
---
...
@Component({
  ...
  selector: 'app-create-<%= h.changeCase.kebabCase(moduleName)%>',
  templateUrl: './create.component.html',
})
export class Create<%= h.changeCase.pascalCase(moduleName)%>Component {
 ...
 // Declare input form
  inputForm = new FormGroup({
    ...
    <%= h.changeCase.camelCase(entityAttributes[i])%>: new FormControl('', Validators.compose([Validators.required, Validators.minLength(1)])),<% } %>
  });
